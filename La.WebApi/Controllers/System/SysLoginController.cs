...
namespace La.WebApi.Controllers.System
{
    ...
    public class SysLoginController : BaseController
    {
        ...
        {
            if (loginBody == null) { throw new CustomException("请求参数错误(The request parameter is incorrect)"); }
            ...
            {
                return ToResponse(ResultCode.CAPTCHA_ERROR, "验证码错误(Captcha error)");
            }
        ...
        public IActionResult Register([FromBody] RegisterDto dto)
        {
            ...
            {
                return ToResponse(ResultCode.CUSTOM_ERROR, "当前系统没有开启注册功能！(New user is not allowed to register now!)");
            }
            SysConfig sysConfig = sysConfigService.GetSysConfigByKey("sys.account.captchaOnOff");
            ...
            return ToResponse(ResultCode.CUSTOM_ERROR, "注册失败，请联系管理员(Registration failed!)");
        }
        ...
        public IActionResult VerifyScan([FromBody] ScanDto dto)
        {
            ...
            if (CacheService.GetScanLogin(dto.Uuid) is Dictionary<string, object> str)
            {
                ...
                if (str.ContainsKey("IsStatus") && (string)str.GetValueOrDefault("IsStatus") == "success")
                {
                    IsStatus = 2;//扫码成功
        ...
        {
            if (dto == null) { return ToResponse(ResultCode.CUSTOM_ERROR, "扫码失败(Scanner failure.)"); }
