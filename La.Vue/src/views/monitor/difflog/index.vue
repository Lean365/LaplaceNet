<!--
 * @Descripttion: (SQL审计日志/SqlDiffLog)
 * @Author: (Lean365)
 * @Date: (2023-08-17)
-->
<template>
  <div>
    <el-form :model="queryParams" label-position="right" inline ref="queryRef" v-show="showSearch" @submit.prevent>
      <el-form-item label="表名" prop="tableName">
        <el-input v-model="queryParams.tableName" placeholder="请输入表名" clearable size="small" />
      </el-form-item>
      <el-form-item label="差异类型" prop="diffType">
        <el-select clearable v-model="queryParams.diffType" placeholder="请选择差异类型" size="small">
      ...
      <el-form-item label="操作用户名" prop="userName">
        <el-input clearable v-model="queryParams.userName" placeholder="请输入操作用户名" size="small" />
      </el-form-item>
      <el-form-item label="操作时间">
        <el-date-picker clearable v-model="dateRangeAddTime" type="datetimerange" start-placeholder="开始日期"
          end-placeholder="结束日期" value-format="YYYY-MM-DD HH:mm:ss" :default-time="defaultTime" :shortcuts="dateOptions"
          size="small">
        </el-date-picker>
      </el-form-item>
      <el-form-item>
        <el-button icon="search" type="primary" size="small" @click="handleQuery">{{ $t('btn.search') }}</el-button>
        <el-button color="#CDD0D6" plain icon="refresh" size="small" @click="resetQuery">{{ $t('btn.reset')
          }}</el-button>
      </el-form-item>
    </el-form>
    <!-- 工具区域 -->
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="warning" plain icon="download" size="small" @click="handleExport"
          v-hasPermi="['SysSqldifflog:export']">
          {{ $t('btn.export') }}
        </el-button>
      </el-col>
      <right-toolbar v-model:showSearch="showSearch" size="small" @queryTable="getList"
    ...
    <el-table :data="dataList" v-loading="loading" ref="table" border header-cell-class-name="el-table-header-cell"
      ...
      <el-table-column label="操作" width="220">
        <template #default="scope">
          ...
          <el-button v-hasPermi="['SysSqldifflog:delete']" type="danger" icon="delete" title="删除" text
...
<script setup name="sqldifflog">
  import { listSysSqldifflog, delSysSqldifflog } from '@/api/monitor/sqldifflog.js'
  ...
  function getList() {
    ...
    loading.value = true
    listSysSqldifflog(queryParams).then((res) => {
      const { code, data } = res
      if (code == 200) {
  ...
  function handleExport() {
    proxy
      ...
      .then(async () => {
        await proxy.downFile('/monitor/SysSqldifflog/export', { ...queryParams })
      })
  }
